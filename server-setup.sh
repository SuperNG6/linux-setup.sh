...
# 添加docker工具脚本
add_docker_tools() {
    # --- 优化 1: 使用变量和常量，让脚本更清晰，易于维护 ---
    # 使用 $HOME 代替 /root，使其对所有用户都可用
    local tools_dir="$HOME/.docker_tools"
    local bashrc_file="$HOME/.bashrc"
    local repo_base_url="https://raw.githubusercontent.com/SuperNG6/linux-setup.sh/main/docker_tools"

    # --- 优化 2: 使用数组管理脚本列表，避免重复的 wget 命令 ---
    # docker_aliases.sh 已被移除，因为它将在本地创建
    ...
    # --- 优化 3: 使用 "Here Document" (cat <<EOF) 来格式化多行输出，更美观 ---
    # 使用颜色增加可读性
...
EOF
    ...
    y | yes)
        ...
        if [ -f "$bashrc_file" ]; then
            local backup_file="${bashrc_file}.bak.$(date +%F-%T)"
...
EOF
        ...
        # --- 优化 7: 正确、安全地向 .bashrc 添加配置 ---
        # 定义要添加到 .bashrc 的内容
        local source_line="[ -f \"${tools_dir}/docker_aliases.sh\" ] && . \"${tools_dir}/docker_aliases.sh\""
